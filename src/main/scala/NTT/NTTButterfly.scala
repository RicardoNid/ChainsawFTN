...
class NTTButterfly(N: BigInt /*被模常数*/) extends Component {
  ...
  val io = new Bundle {
    ...
    val C = in Bits (n bits)
    val D = out Bits (n bits) //D=C+A*B*2^-n
    val E = out Bits (n bits) //E=C-A*B*2^-n

  }

  val my_tree = MontMul(io.A, io.B, N)
  val my_tree1 = WallaceTree().insert(io.C, 0).add(my_tree)
  val my_tree4 = WallaceTree().insert(io.C, 0).sub(my_tree)
  ...
  val output1 = my_tree1.part().full().conv_Bits(0, n)
  val output2 = my_tree2.part().full().conv_Bits(0, n + 1)
  val output3 = my_tree3.part().full().conv_Bits(0, n + 2)
  val output4 = my_tree4.part().full().conv_Bits(0, n + 2)
  val output5 = my_tree5.part().full().conv_Bits(0, n + 1)
  val output6 = my_tree6.part().full().conv_Bits(0, n)
